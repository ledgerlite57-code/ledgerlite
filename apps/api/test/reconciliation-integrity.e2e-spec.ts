...
describe("Reconciliation integrity (e2e)", () => {
  ...
  const createAuthContext = async () => {
    ...
    const user = await prisma.user.create({
      data: { email: "recon@ledgerlite.local", passwordHash: "hash" },
    });
    ...
    return { org, user, token, bankAccount, bankGlAccount, cashAccount, revenueAccount };
  };
  ...
  it("blocks overlapping reconciliation sessions for the same bank account", async () => {
    const { token, bankAccount } = await createAuthContext();

    await createSession(token, bankAccount.id, "2026-01-01", "2026-01-31");
  ...
  it("marks bank transactions as matched after successful reconciliation", async () => {
    ...
    const glHeader = await createGlHeader(org.id, user.id, "2026-01-10T00:00:00.000Z", "JOURNAL-1", [
  ...
  it("blocks matching when GL header does not affect the bank account", async () => {
    const { org, user, token, bankAccount, cashAccount, revenueAccount } = await createAuthContext();
    ...
    const glHeader = await createGlHeader(org.id, user.id, "2026-01-10T00:00:00.000Z", "JOURNAL-2", [
      { accountId: cashAccount.id, debit: 50, credit: 0 },
  ...
  it("blocks matching when GL posting date is outside the reconciliation period", async () => {
    const { org, user, token, bankAccount, bankGlAccount, revenueAccount } = await createAuthContext();
    ...
    const glHeader = await createGlHeader(org.id, user.id, "2026-02-10T00:00:00.000Z", "JOURNAL-3", [
      { accountId: bankGlAccount.id, debit: 75, credit: 0 },
      { accountId: revenueAccount.id, debit: 0, credit: 75 },
    ]);
  ...
  it("blocks matching reversed GL headers", async () => {
    const { org, user, token, bankAccount, bankGlAccount, revenueAccount } = await createAuthContext();
    ...
    const glHeader = await createGlHeader(org.id, user.id, "2026-01-10T00:00:00.000Z", "JOURNAL-4", [
      { accountId: bankGlAccount.id, debit: 120, credit: 0 },
      { accountId: revenueAccount.id, debit: 0, credit: 120 },
    ]);

    await prisma.$transaction((tx) => createGlReversal(tx, glHeader.id, user.id, { memo: "Reverse entry" }));
  ...
  it("blocks matching a bank transaction already reconciled in another session", async () => {
    const { org, user, token, bankAccount, bankGlAccount, revenueAccount } = await createAuthContext();
    ...
    const glHeader = await createGlHeader(org.id, user.id, "2026-01-10T00:00:00.000Z", "JOURNAL-5", [
      { accountId: bankGlAccount.id, debit: 200, credit: 0 },
    ...
    const overlappingSession = await prisma.reconciliationSession.create({
      data: {
        orgId: org.id,
        bankAccountId: bankAccount.id,
        periodStart: new Date("2026-01-05T00:00:00.000Z"),
        periodEnd: new Date("2026-01-20T00:00:00.000Z"),
        statementOpeningBalance: new Prisma.Decimal(0),
    ...
    await request(app.getHttpServer())
      .post(`/reconciliation-sessions/${overlappingSession.id}/match`)
      .set("Authorization", `Bearer ${token}`)
      .send({ bankTransactionId: bankTransaction.id, glHeaderId: glHeader.id })
      .expect(409);
  });
