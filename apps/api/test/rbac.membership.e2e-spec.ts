...
describe("RBAC membership validation (e2e)", () => {
  ...
  it("rejects protected requests when membership is inactive", async () => {
    await prisma.permission.create({
      data: { code: Permissions.ORG_READ, description: Permissions.ORG_READ },
    });

    const org = await prisma.organization.create({
      data: { name: "RBAC Org", baseCurrency: "AED" },
    });
    ...
    const user = await prisma.user.create({
      data: { email: "rbac@ledgerlite.local", passwordHash: "hash" },
    });
