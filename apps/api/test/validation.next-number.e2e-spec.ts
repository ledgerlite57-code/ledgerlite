...
describe("Numbering validation (e2e)", () => {
  ...
  const seedOrg = async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: "Numbering Org", baseCurrency: "AED", countryCode: "AE", timeZone: "UTC", vatEnabled: false },
    });
    ...
    const user = await prisma.user.create({
      data: { email: `numbering-${Date.now()}@ledgerlite.local`, passwordHash: "hash" },
    });
  ...
  it("rejects nextNumber below 1", async () => {
    const { token } = await seedOrg();

    await request(app.getHttpServer())
      .patch("/orgs/settings")
      .set("Authorization", `Bearer ${token}`)
      .send({ invoiceNextNumber: 0 })
      .expect(400);
  });
});
