...
describe("Exchange rate validation (e2e)", () => {
  ...
  const seedOrg = async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: "FX Org", baseCurrency: "AED", countryCode: "AE", timeZone: "UTC", vatEnabled: false },
    });
    ...
    const user = await prisma.user.create({
      data: { email: `fx-${Date.now()}@ledgerlite.local`, passwordHash: "hash" },
    });
  ...
  it("rejects exchangeRate=0", async () => {
