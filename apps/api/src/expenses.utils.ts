...
export type CalculatedExpenseLine = {
  lineNo: number;
  expenseAccountId: string;
  itemId?: string | null;
...
export function calculateExpenseLines(params: {
  ...
  const calculated: CalculatedExpenseLine[] = [];
  let subTotal = dec(0);
  let taxTotal = dec(0);
...
export function buildExpensePostingLines(params: {
  ...
  for (const line of params.lines) {
    if (!line.expenseAccountId) {
      throw new Error("Expense lines must reference an expense account to post");
    }
  ...
  let lineNo = 1;
  const description = params.expenseNumber ? `Expense ${params.expenseNumber}` : "Expense";
