...
describe("Pagination units (e2e)", () => {
  ...
  const seedOrg = async (permissions: string[]) => {
    ...
    const org = await prisma.organization.create({
      data: { name: "Pagination Units Org", baseCurrency: "AED", countryCode: "AE", timeZone: "Asia/Dubai", vatEnabled: false },
    });
    ...
    const user = await prisma.user.create({
      data: { email: `units-${Date.now()}@ledgerlite.local`, passwordHash: "hash" },
    });
  ...
  it("paginates and filters units", async () => {
    const { org, token } = await seedOrg([Permissions.ITEM_READ]);

    const each = await prisma.unitOfMeasure.create({
      data: {
        orgId: org.id,
        name: "Each",
        symbol: "ea",
    ...
    const page1 = await request(app.getHttpServer())
      .get("/units-of-measurement?page=1&pageSize=2")
    ...
    const page2 = await request(app.getHttpServer())
      .get("/units-of-measurement?page=2&pageSize=2")
      .set("Authorization", `Bearer ${token}`)
      .expect(200);
    ...
    const inactive = await request(app.getHttpServer())
      .get("/units-of-measurement?isActive=false")
      .set("Authorization", `Bearer ${token}`)
      .expect(200);
    ...
    const search = await request(app.getHttpServer())
      .get("/units-of-measurement?search=doz")
      .set("Authorization", `Bearer ${token}`)
      .expect(200);
