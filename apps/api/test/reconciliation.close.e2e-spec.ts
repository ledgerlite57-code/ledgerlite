...
describe("Reconciliation close (e2e)", () => {
  ...
  const createAuthContext = async () => {
    ...
    const user = await prisma.user.create({
      data: { email: "recon-close@ledgerlite.local", passwordHash: "hash" },
    });
  ...
  it("blocks closing when statement difference is not zero", async () => {
    ...
    const glHeader = await createGlHeader(org.id, user.id, "2026-01-10T00:00:00.000Z", "JOURNAL-CLOSE-1", [
      { accountId: bankGlAccount.id, debit: 50, credit: 0 },
    ...
    await request(app.getHttpServer())
      .post(`/reconciliation-sessions/${session.id}/close`)
      .set("Authorization", `Bearer ${token}`)
      .send({})
      .expect(409);
  });

  it("closes when statement difference is zero", async () => {
    const { org, user, token, bankAccount, bankGlAccount, revenueAccount } = await createAuthContext();
    ...
    const glHeader = await createGlHeader(org.id, user.id, "2026-01-12T00:00:00.000Z", "JOURNAL-CLOSE-2", [
