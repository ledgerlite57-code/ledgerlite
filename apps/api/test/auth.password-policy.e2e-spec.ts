...
describe("Auth password policy (e2e)", () => {
  ...
  it("rejects weak passwords during invite acceptance", async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: "Password Policy Org", baseCurrency: "AED" },
    });
    ...
    const user = await prisma.user.create({
      data: { email: "policy-owner@ledgerlite.local", passwordHash: "hash" },
    });
    ...
    const inviteRes = await request(app.getHttpServer())
      ...
      .send({ email: "weak-pass@ledgerlite.local", roleId: role.id })
      .expect(201);
