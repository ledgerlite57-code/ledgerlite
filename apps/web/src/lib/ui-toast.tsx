...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Viewport
    ref={ref}
    className={cn("fixed bottom-0 right-0 z-50 flex max-h-screen w-full flex-col p-4 sm:max-w-sm", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Close
    ref={ref}
    className={cn("absolute right-2 top-2 rounded-md p-1 text-foreground/60 hover:text-foreground", className)}
    {...props}
  >
    <X className="h-4 w-4" />
  </ToastPrimitive.Close>
