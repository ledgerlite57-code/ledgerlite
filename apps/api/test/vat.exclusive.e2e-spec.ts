...
describe("VAT exclusive behavior (e2e)", () => {
  ...
  const seedOrg = async () => {
    ...
    const org = await prisma.organization.create({
      data: { name: "VAT Exclusive Org", baseCurrency: "AED", countryCode: "AE", timeZone: "UTC", vatEnabled: true },
    });

    await prisma.orgSettings.create({
      data: { orgId: org.id, defaultVatBehavior: "EXCLUSIVE" },
    });
    ...
    const user = await prisma.user.create({
      data: { email: `vat-exclusive-${Date.now()}@ledgerlite.local`, passwordHash: "hash" },
    });
  ...
  it("adds tax for exclusive prices", async () => {
